...
export interface AttendanceSummary {
  workDays: number    // วันทำงาน (ไม่รวมวันหยุด)
  normalDays: number
  lateDays: number
  absentDays: number
  leaveDays: number
...
const STATUS_BG: Record<string, string> = {
  ...
  absent:          '#fee2e2',
  'leave-sick':    '#ffedd5',
  'leave-vacation':'#dbeafe',
  'leave-personal':'#cffafe',
  holiday:         '#f1f5f9',
}
...
const STATUS_COLOR: Record<string, string> = {
  ...
  absent:          '#991b1b',
  'leave-sick':    '#9a3412',
  'leave-vacation':'#1e40af',
  'leave-personal':'#155e75',
  holiday:         '#64748b',
}
...
const s = StyleSheet.create({
  ...
  hospitalSub:  { fontSize: 8,  color: 'rgba(255,255,255,0.8)', marginTop: 2 },
  reportTitle:  { fontSize: 10, fontWeight: 'bold', color: '#fff', textAlign: 'right' },
  ...
  summaryCard: {
    ...
    padding: '5px 8px',
    alignItems: 'center',
    borderTop: '2px solid #006a5a',
  },
  summaryNum: { fontSize: 14, fontWeight: 'bold', color: '#006a5a' },
  ...
  footNote: { fontSize: 6.5, color: '#94a3b8', textAlign: 'right', marginTop: 6 },
})
...
function AttendanceDocument({ data }: { data: AttendanceReportData }) {
  const { employee, monthLabel, records, summary } = data
