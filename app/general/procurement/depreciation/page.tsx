...
import {
  ...
  Segmented, Statistic, Tag, Progress, Select, Tooltip, Tabs, Descriptions,
} from 'antd'
import {
  HomeOutlined, FileTextOutlined, SearchOutlined, CalendarOutlined,
  ReloadOutlined, QrcodeOutlined,
...
import { CLASS_OPTIONS, classByNo, classForAssetcatid, type AssetClass } from './assetClasses'
...
const PageContent = () => {
  ...
  const handleSelectAsset = (asset: EquipmentAsset) => {
    ...
    setClassNo(classForAssetcatid(asset.assetcatid)?.no ?? null)
  ...
  // ── GFMIS: เกณฑ์นับเดือน ──
  // ไม่กันราคาซาก เพราะ GFMIS คิดค่าเสื่อมจนหมดมูลค่า (ตรงกับตัวอย่างที่ใช้ตรวจสูตร)
  const gfmis = useMemo(() => {
    if (!selected?.receive || !selected.perunits || !lifeYears) return null
    return computeGfmis(
      { cost: Number(selected.perunits), lifeYears: Number(lifeYears), receive: selected.receive, residual: 0 },
      asOfIso,
    )
  }, [selected, lifeYears, asOfIso])
  ...
  const registryClass = classForAssetcatid(selected?.assetcatid)
    ...
    <div style={{ minHeight: '100dvh', background: 'var(--app-bg)' }}>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '20px 20px 48px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
          ...
          <Title level={3} style={{ margin: 0 }}>คำนวณค่าเสื่อมราคาครุภัณฑ์</Title>
        </div>
        <Text type="secondary" style={{ display: 'block', marginBottom: 20 }}>
          วิธีเส้นตรง (straight-line) ตามปีงบประมาณไทย 1 ต.ค. – 30 ก.ย.
        </Text>
                    ...
                    children: gfmis ? (
                      <>
                        <Alert
                          ...
                          description={`เริ่มคิด ${gfmis.startMonth} ถึง ${gfmis.endMonth} รวม ${gfmis.totalMonths} เดือน · เดือนละ ${money(gfmis.perMonth)} บาท · คิดถึง ${asOfIso}`}
                        />
